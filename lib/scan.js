 ...
 */
function findStringInReactComponents(source, file) {
    var tokens = parse(source, file);
    var formattedMessageFound = false;
    var strings = [];
        ...
        async.reduce(files, tokens, function (tokens, file, next) {
            fs.readFile(file, function (err, src) {
                ...
                if (path.extname(file) === '.jsx') {
                    tokens[scope] = tokens[scope].concat(findStringInReactComponents(source, file));
                }
                return next(null, tokens);
            });
        }, done);
